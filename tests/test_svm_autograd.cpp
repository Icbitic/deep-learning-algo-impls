...
TEST_F(SVMAutogradTest, AutogradBasicTest) {
    ...
    // Test addition
    Variable<double> result = var1 + var2;
...
TEST_F(SVMAutogradTest, ConstructorTest) {
    ...
    EXPECT_THROW(SVM<double> svm(KernelType::LINEAR, 1.0, 1.0, 3, 0.0, 1e-3, 1000, -0.01), 
...
TEST_F(SVMAutogradTest, ComparisonWithOriginalSVMTest) {
    ...
    // Autograd SVM
    SVM<double> autograd_svm(KernelType::LINEAR, 1.0, 1.0, 3, 0.0, 1e-3, 200, 0.01);
    ...
    auto duration_autograd = std::chrono::duration_cast<std::chrono::milliseconds>(end_autograd - start_autograd);
    ...
    for (size_t i = 0; i < y_linear_.size(); ++i) {
        if (original_predictions[i] == y_linear_[i]) original_correct++;
        if (autograd_predictions[i] == y_linear_[i]) autograd_correct++;
    }
    
    double original_accuracy = static_cast<double>(original_correct) / y_linear_.size();
    double autograd_accuracy = static_cast<double>(autograd_correct) / y_linear_.size();
    
    std::cout << "Original SVM accuracy: " << original_accuracy << ", Time: " << duration_original.count() << "ms" << std::endl;
    std::cout << "Autograd SVM accuracy: " << autograd_accuracy << ", Time: " << duration_autograd.count() << "ms" << std::endl;
