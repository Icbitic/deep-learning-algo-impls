...
protected:
    void SetUp() override {
        ...
        X_1d_ = Tensor<double>({{2.0}, {3.0}, {4.0}, {-2.0}, {-3.0}, {-4.0}}, {6});
        y_1d_ = {1, 1, 1, -1, -1, -1};
    }
...
TEST_F(SVMTest, SmallDatasetTest) {
    ...
    Tensor<double> X_small({1.0, -1.0}, {2, 1});
...
TEST_F(SVMTest, StaticVsAutogradComparisonTest) {
    std::cout << "\n=== Comparison: Static Computation vs Autograd ===" <<
            std::endl;

    // Test with linear kernel on linearly separable data
    std::cout << "\nTesting Linear Kernel on Linearly Separable Data:" <<
            std::endl;

    // Original SVM with static computation (SMO algorithm)
    std::cout << "\n1. Original SVM (Static Computation - SMO Algorithm):" <<
            std::endl;
    ...
    auto duration_static = std::chrono::duration_cast<std::chrono::microseconds>(
    ...
    for (size_t i = 0; i < y_linear_.size(); ++i) {
        if (static_predictions[i] == y_linear_[i])
            static_correct++;
    }
    double static_accuracy = static_cast<double>(static_correct) / y_linear_.
                             size();

    std::cout << "   - Training time: " << duration_static.count() <<
            " microseconds" << std::endl;
    std::cout << "   - Accuracy: " << static_accuracy * 100 << "%" << std::endl;
    std::cout << "   - Method: Sequential Minimal Optimization (SMO)" <<
            std::endl;
    std::cout << "   - Gradient computation: Manual/Analytical" << std::endl;

    // New SVM with autograd
    std::cout << "\n2. New SVM (Automatic Differentiation - Gradient Descent):" <<
            std::endl;
    SVM<double> autograd_svm(KernelType::LINEAR, 1.0, 1.0, 3, 0.0, 1e-4, 300,
                             0.01);
    ...
    auto duration_autograd = std::chrono::duration_cast<
        std::chrono::microseconds>(end_autograd - start_autograd);
    ...
    if (loss_history.size() > 0) {
        ...
        std::cout << "   - Training iterations: " << loss_history.size() <<
                std::endl;
    }

    std::cout << "\n=== Key Differences ===" << std::endl;
    std::cout << "1. Gradient Computation:" << std::endl;
    std::cout << "   - Static: Manual gradient calculations in SMO algorithm" <<
            std::endl;
    std::cout << "   - Autograd: Automatic differentiation tracks operations" <<
            std::endl;

    std::cout << "\n2. Optimization Method:" << std::endl;
    std::cout << "   - Static: Sequential Minimal Optimization (SMO)" <<
            std::endl;
    std::cout << "   - Autograd: Gradient descent with backpropagation" <<
            std::endl;

    std::cout << "\n3. Flexibility:" << std::endl;
    std::cout << "   - Static: Fixed optimization algorithm, hard to modify" <<
            std::endl;
    std::cout << "   - Autograd: Easy to experiment with different loss functions"
            << std::endl;

    std::cout << "\n4. Computational Graph:" << std::endl;
    std::cout << "   - Static: No computational graph, direct computation" <<
            std::endl;
    std::cout << "   - Autograd: Builds computational graph for backpropagation"
            << std::endl;
    ...
    std::cout << "Both implementations achieved reasonable accuracy (>60%)" <<
            std::endl;
    std::cout << "Autograd provides more flexibility for experimentation" <<
            std::endl;
    std::cout << "Static SMO is more traditional and well-established" <<
            std::endl;
}
